...
const expirationDatePatternErrorMessage =
  'can be only in second, minute, hour, day and month';
...
export const validationSchema = Joi.object({
  ...
  [EnvironmentVariableNames.RATE_LIMITER_DEFAULT_TTL]:
    Joi.number().default(60000),
  [EnvironmentVariableNames.RATE_LIMITER_DEFAULT_LIMIT]:
  ...
  [EnvironmentVariableNames.LOGIN_RATE_LIMITER_DEFAULT_LIMIT]:
    Joi.number().default(5),
  [EnvironmentVariableNames.VERIFY_SET_FIRST_PASSWORD_RATE_LIMITER_DEFAULT_TTL]:
    Joi.number().default(60000),
  [EnvironmentVariableNames.VERIFY_SET_FIRST_PASSWORD_RATE_LIMITER_DEFAULT_LIMIT]:
    Joi.number().default(5),
  [EnvironmentVariableNames.FORGET_PASSWORD_RATE_LIMITER_DEFAULT_TTL]:
    Joi.number().default(60000),
  [EnvironmentVariableNames.FORGET_PASSWORD_RATE_LIMITER_DEFAULT_LIMIT]:
    Joi.number().default(5),
  [EnvironmentVariableNames.VERIFY_RESET_PASSWORD_RATE_LIMITER_DEFAULT_TTL]:
    Joi.number().default(60000),
  [EnvironmentVariableNames.VERIFY_RESET_PASSWORD_RATE_LIMITER_DEFAULT_LIMIT]:
    Joi.number().default(5),
  ...
  [EnvironmentVariableNames.EMAIL_PROVIDER_PORT]: Joi.number()
    .port()
    .required(),
  ...
  [EnvironmentVariableNames.MAIN_APP_CHANGE_EMAIL_ROUTE]: Joi.string()
    .uri()
    .required(),
  [EnvironmentVariableNames.MAIN_APP_OAUTH_CALLBACK_URL]: Joi.string()
    .uri()
    .required(),

  [EnvironmentVariableNames.BULLMQ_RESET_PASSWORD_QUEUE]:
    Joi.string().required(),
  [EnvironmentVariableNames.BULLMQ_EMAIL_VERIFICATION_QUEUE]:
    Joi.string().required(),
  [EnvironmentVariableNames.BULLMQ_SET_FIRST_PASSWORD_QUEUE]:
    Joi.string().required(),
  [EnvironmentVariableNames.BULLMQ_CHANGE_EMAIL_QUEUE]: Joi.string().required(),
  [EnvironmentVariableNames.BULLMQ_PASSWORD_CHANGED_QUEUE]:
    Joi.string().required(),
  ...
  [EnvironmentVariableNames.GOOGLE_OAUTH_CLIENT_SECRET]:
    Joi.string().required(),
  [EnvironmentVariableNames.GOOGLE_OAUTH_CALLBACK_URL]: Joi.string()
    .uri()
    .required(),
