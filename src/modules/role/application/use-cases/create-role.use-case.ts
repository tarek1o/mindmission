...
export class CreateRoleUseCase {
  ...
  private buildRoleTranslationModels(
  ...
  private async create(
    translationsInput: RoleTranslationInput[],
    ...
    return this.unitOfWork.transaction(async (manager) => {
      ...
      const translationsModels = this.buildRoleTranslationModels(
        translationsInput,
        role.id,
      );
      const translations = await this.roleTranslationRepository.saveMany(
        translationsModels,
        manager,
      );
      return {
  ...
  async execute(input: RoleInput): Promise<RoleWithTranslationsViewModel> {
    ...
    const permissions = await this.roleValidationService.getPermissionsByIds(
      input.permissionIds,
    );
    const roleWithTranslationsViewModel = await this.create(
      input.translations,
      permissions,
    );
