...
export class UpdateRoleUseCase {
  ...
  private upsertTranslations(translations: RoleTranslationModel[], translationProps: RoleTranslationInput[]): RoleTranslationModel[] {
    ...
    translationProps.forEach(props => {
      ...
      if(existingTranslation) {
        ...
        updatedTranslations.push(new RoleTranslationModel({
          ...props,
          roleId,
        }));
      }
  ...
  private async validate(id: number, translations: RoleTranslationModel[], permissionIds: number[]): Promise<void> {
  ...
  private getDeletedTranslations(currentTranslations: RoleTranslationModel[], newTranslations: RoleTranslationModel[]): RoleTranslationModel[] {
    return currentTranslations.filter(t => !newTranslations.some(t2 => t2.language === t.language));
  }

  private async save(role: RoleModel, translations: RoleTranslationModel[], deletedTranslations: RoleTranslationModel[]): Promise<RoleWithTranslationsViewModel> {
    return this.unitOfWork.transaction(async (manager) => {
      const savedRole = await this.roleRepository.save(role, manager);
      const savedTranslations = await this.roleTranslationRepository.saveMany(translations, manager);
      deletedTranslations.length && await this.roleTranslationRepository.deleteMany(deletedTranslations, manager);
      return {
        role: savedRole,
  ...
  async execute(id: number, input: Partial<RoleInput>): Promise<RoleWithTranslationsViewModel> {
    const { role, translations } = await this.roleFinderService.getWithTranslationsById(id);
    role.permissions = input.permissionIds?.length ? await this.validationService.getPermissionsByIds(input.permissionIds) : role.permissions; 
    const translationsModels = this.upsertTranslations(translations, input.translations);
    await this.validate(id, translationsModels, input.permissionIds ?? []);
    const deletedTranslations = this.getDeletedTranslations(translations, translationsModels);
    const roleWithTranslationsViewModel = await this.save(role, translationsModels, deletedTranslations);
