...
export class NotificationChannelFinderService {
  ...
  async getChannel(event: NotificationEventEnum): Promise<NotificationChannelEnum> {
    return (await this.notificationSettingsCacheService.getOne(event))?.notificationChannel 
      ?? (await this.notificationSettingsRepository.getByEvent(event))?.notificationChannel
      ?? this.configService.get<SettingsConfiguration>('settings').notifications.defaultChannel;
  }
}