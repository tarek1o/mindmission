...
export class NotificationRepository implements INotificationRepository {
  ...
  async save(notification: NotificationModel, manager?: EntityManager): Promise<NotificationModel> {
    const entity = NotificationMapper.toEntity(notification);
    const notificationRepository = manager?.getRepository(NotificationEntity) ?? this.notificationRepository;
