...
export class NotificationSettingsController {
  ...
  @AcceptedCriteria({
    privileges: [
      {
        resource: ResourceEnum.NOTIFICATION_SETTINGS,
  ...
  async getAll(
    ...
    const { models, count } =
      await this.getAllNotificationSettingsUseCase.execute(
        query,
        order,
        pagination,
      );
    return {
      data: models.map(
        (notificationSettings) =>
          new NotificationSettingsResponseDto(notificationSettings),
      ),
      count,
    };
  }
  ...
  async getById(
    ...
    const notificationSettings =
      await this.getNotificationSettingsByIdUseCase.execute(id);
  ...
  async update(
    ...
    const updatedNotificationSettings =
      await this.updateNotificationSettingsUseCase.execute(
        id,
