...
export class NotificationSettingsValidatorService {
  ...
  async validateNotificationEventNotDuplicate(
    notificationEvent: NotificationEventEnum,
    id?: number,
  ): Promise<void> {
    const existingNotificationSettings =
      await this.notificationSettingsRepository.getByEvent(notificationEvent);
    if (
      existingNotificationSettings &&
      existingNotificationSettings.id !== id
    ) {
      throw new ConflictError(
        'notification_settings.notification_event.duplicate',
      );
    }
  }
}
