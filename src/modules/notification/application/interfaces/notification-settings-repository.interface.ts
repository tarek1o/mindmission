...
export interface INotificationSettingsRepository {
  getAllPaginatedAndTotalCount(query: GetAllNotificationSettingsQueryInput, order: IOrder<AllowedNotificationSettingsOrderColumnsEnum>, pagination: Pagination): Promise<{ models: NotificationSettingsModel[], count: number }>;
  getById(id: number): Promise<NotificationSettingsModel | null>;
  getByEvent(event: NotificationEventEnum): Promise<NotificationSettingsModel | null>;
  save(model: NotificationSettingsModel, manager?: unknown): Promise<NotificationSettingsModel>;
}