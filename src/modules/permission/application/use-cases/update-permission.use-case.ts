...
export class UpdatePermissionUseCase {
  constructor(
    ...
    @Inject(PERMISSION_TRANSLATION_REPOSITORY) private permissionTranslationRepository : IPermissionTranslationRepository,
    @Inject(UNIT_OF_WORK) private readonly unitOfWork: IUnitOfWork,
    @Inject(PERMISSION_CACHE) private readonly cacheService: IPermissionCacheService,
  ) {}

  private upsertTranslations(translations: PermissionTranslationModel[], translationProps: PermissionTranslationInput[]): PermissionTranslationModel[] {
    ...
    translationProps.forEach(props => {
      ...
      if(existingTranslation) {
        ...
        updatedTranslations.push(new PermissionTranslationModel({
          ...props,
          permissionId,
        }));
      }
  ...
  private save(permissionModel: PermissionModel, upsertTranslationsModels: PermissionTranslationModel[], deletedTranslationsModels: PermissionTranslationModel[]): Promise<PermissionWithTranslationsViewModel> {
    return this.unitOfWork.transaction(async (manager) => {
      const permission = await this.permissionRepository.save(permissionModel, manager);
      const translations = await this.permissionTranslationRepository.saveMany(upsertTranslationsModels, manager);
      deletedTranslationsModels.length && await this.permissionTranslationRepository.deleteMany(deletedTranslationsModels, manager);
      return {
        permission,
        translations,
      };
  ...
  private async validate(permission: PermissionModel, newTranslationsModels: PermissionTranslationModel[]): Promise<void> {
    const validationInput = {
      resource: permission.resource,
      actions: permission.actions,
      translations: newTranslationsModels.map(t => ({ language: t.language, name: t.name })),
    }
    await this.permissionValidationService.validate(validationInput, permission.id);
  }

  private getDeletedTranslations(currentTranslations: PermissionTranslationModel[], newTranslations: PermissionTranslationModel[]): PermissionTranslationModel[] {
  ...
  private recacheUpdatedPermission(permission: PermissionModel): Promise<void> {
    const permissionCacheViewModel = PermissionCacheMapper.toCacheViewModel(permission)
    return this.cacheService.saveOne(permissionCacheViewModel);
  }

  async execute(id: number, input: Partial<PermissionInput>): Promise<PermissionWithTranslationsViewModel> {
    const { permission, translations } = await this.permissionFinderService.getWithTranslationsById(id);
    permission.update(input);
    const newTranslationsModels = this.upsertTranslations(translations, input.translations);
    await this.validate(permission, newTranslationsModels);
    const deletedTranslations = this.getDeletedTranslations(translations, newTranslationsModels);
    const permissionWithTranslationsViewModel = await this.save(permission, newTranslationsModels, deletedTranslations);
