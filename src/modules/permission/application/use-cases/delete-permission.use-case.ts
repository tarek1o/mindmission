...
export class DeletePermissionUseCase {
  ...
  private async checkIfRoleHasOnlyThisPermission(permissionId: number): Promise<void> {
    const count = await this.permissionRepository.countRolesWithOnlyPermission(permissionId);
    if(count) {
      this.loggerService.error(`The permission with id: ${permissionId} is used in a role and this role has this permission only`);
      throw new BusinessRuleViolationError('permission.role_has_only_this_permission', { id: permissionId });
    }
  }

  private save(permission: PermissionModel, translations: PermissionTranslationModel[]): Promise<void> {
    return this.unitOfWork.transaction(async (manager) => {
      await this.permissionRepository.save(permission, manager);
      await this.permissionTranslationRepository.saveMany(translations, manager);
    });
  }
