...
export class CreatePermissionUseCase {
  ...
  private buildPermissionTranslationModels(translations: PermissionTranslationInput[], permissionId: number): PermissionTranslationModel[] {
    return translations.map(translation => new PermissionTranslationModel({
      ...translation,
      permissionId,
    }));
  }

  private create(input: PermissionInput): Promise<PermissionWithTranslationsViewModel> {
    return this.unitOfWork.transaction(async (manager) => {
      ...
      const translationsModels = this.buildPermissionTranslationModels(input.translations, permission.id);
      const translations = await this.permissionTranslationRepository.saveMany(translationsModels, manager);
      return {
        permission,
        translations,
      };
  ...
  async execute(input: PermissionInput): Promise<PermissionWithTranslationsViewModel> {
    await this.permissionValidationService.validate(input);
    const permissionWithTranslationsViewModel = await this.create(input);
    await this.cacheCreatedPermission(permissionWithTranslationsViewModel.permission);
