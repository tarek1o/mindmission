...
export class PermissionValidationService {
  constructor(
    ...
    @Inject(PERMISSION_TRANSLATION_REPOSITORY) private readonly permissionTranslationRepository: IPermissionTranslationRepository,
    @Inject(LOGGER_SERVICE) private readonly logger: LoggerService,
  ) {}

  private checkEnglishAndArabicTranslations(translations: PermissionTranslationInput[]): void {
  ...
  private checkDuplicateNames(duplicates: GetPermissionTranslationByNameViewModel[]): void {
  ...
  private async checkPermissionNamesDuplicate(translationsProps: PermissionTranslationInput[], permissionId?: number) {
    const parameters = translationsProps.map(({ language, name }) => ({ language, name }));
    const permissionTranslations = await this.permissionTranslationRepository.getByNameAndLanguageExcludingPermissionId(parameters, permissionId);
    this.checkDuplicateNames(permissionTranslations);
  }

  private async checkPermissionResourceActionsDuplicate(permissionProps: PermissionProps, permissionId?: number) {
    const { resource, actions } = permissionProps;
    const duplicateCount = await this.permissionRepository.getCountByResourceAndActions(resource, actions, permissionId);
    if (duplicateCount) {
      throw new ConflictError('permission.resource_actions.duplicate');
    }
  }

  async validate(permissionInput: PermissionInput, permissionId?: number): Promise<void> {
    this.checkEnglishAndArabicTranslations(permissionInput.translations);
    await Promise.all([
      this.checkPermissionNamesDuplicate(permissionInput.translations, permissionId),
      this.checkPermissionResourceActionsDuplicate(permissionInput, permissionId),
    ]);
  }
}
