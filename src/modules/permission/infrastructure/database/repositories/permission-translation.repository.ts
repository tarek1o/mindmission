...
export class PermissionTranslationRepository implements IPermissionTranslationRepository {
  constructor(
    @InjectRepository(PermissionTranslationEntity)
    private readonly permissionTranslationEntity: Repository<PermissionTranslationEntity>,
    @Inject(LOGGER_SERVICE) private readonly loggerService: LoggerService,
  ) {}

  async getByLanguage(
    language: LanguageEnum,
  ): Promise<GetPermissionTranslationByLanguageViewModel[]> {
    const permissionTranslationEntities =
      await this.permissionTranslationEntity.find({
        where: {
          language,
        },
        select: {
    ...
    return permissionTranslationEntities.map(({ name, permissionId }) => ({
      name,
      permissionId,
    }));
  }

  async getByNameAndLanguageExcludingPermissionId(
    translations: { language: LanguageEnum; name: string }[],
    permissionId?: number,
  ): Promise<GetPermissionTranslationByNameViewModel[]> {
    const qb =
      this.permissionTranslationEntity.createQueryBuilder('translations');
    ...
    const permissionTranslationEntities = await qb
      .select(['translations.language', 'translations.name'])
      .getMany();

    return permissionTranslationEntities.map(({ language, name }) => ({
      language,
      name,
    }));
  }
  ...
  ): Promise<PermissionTranslationModel[]> {
    try {
      const permissionTranslationEntities = translations.map((translation) =>
        PermissionTranslationMapper.toEntity(translation),
      );
      const permissionTranslationEntity =
        manager?.getRepository(PermissionTranslationEntity) ??
        this.permissionTranslationEntity;
      const savedPermissionTranslationEntities =
        await permissionTranslationEntity.save(permissionTranslationEntities);
      return savedPermissionTranslationEntities.map((entity) =>
        PermissionTranslationMapper.toModel(entity),
      );
    } catch (error: unknown) {
  ...
  async deleteMany(
    ...
    if (deleteResults.affected !== translations.length) {
      this.loggerService.warn(
        `Failed to delete ${deleteResults.affected}/${translations.length} permission translations`,
        PermissionTranslationRepository.name,
      );
    }
  }
}
