...
export class UpdateCategoryUseCase {
  ...
  private upsertTranslations(
    ...
    translationProps: CategoryTranslationInput[],
  ...
  private getDeletedTranslations(
    currentTranslations: CategoryTranslationModel[],
    newTranslations: CategoryTranslationModel[],
  ): CategoryTranslationModel[] {
  ...
  private async save(
    ...
    translations: CategoryTranslationModel[],
    deletedTranslations: CategoryTranslationModel[],
    ...
    return this.unitOfWork.transaction(async (manager) => {
      ...
      const savedTranslations =
        await this.categoryTranslationRepository.saveMany(
          translations,
          manager,
        );
      deletedTranslations.length &&
        (await this.categoryTranslationRepository.deleteMany(
          deletedTranslations,
          manager,
        ));
      return {
  ...
  async execute(
    ...
    const translationsModels = input.translations
      ? this.upsertTranslations(translations, input.translations)
      : translations;
    await this.validate(id, translationsModels, input);
    const deletedTranslations = input.translations
      ? this.getDeletedTranslations(translations, translationsModels)
      : [];
    const categoryWithTranslationsViewModel = await this.save(
      category,
      translationsModels,
      deletedTranslations,
    );
