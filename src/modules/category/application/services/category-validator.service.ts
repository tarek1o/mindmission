...
export class CategoryValidatorService {
  ...
  private checkParentExists(
    childType: CategoryTypeEnum,
    parent: CategoryModel | null,
  ): void {
    if (!parent) {
      throw new InvalidInputError('category.parent_id.required', {
        type: childType,
        parent: CATEGORY_PARENT_MAP[childType],
      });
    }
  }
  ...
  private checkParentType(
    ...
    if (parent.type !== parentType) {
      const errorCode = parentType
        ? 'category.parent_id.required'
        : 'category.parent_id.not_allowed';
  ...
  private checkDuplicateNames(
    duplicates: GetCategoryTranslationByNameViewModel[],
  ): void {
  ...
  private async validateDuplicateNames(
    type: CategoryTypeEnum,
    categoryTranslationsProps: CategoryTranslationInput[],
    categoryId?: number,
  ): Promise<void> {
    const parameters = categoryTranslationsProps.map(({ language, name }) => ({
      language,
      name,
    }));
    const categoryTranslations =
      await this.categoryTranslationRepository.getByNameAndLanguageAndTypeExcludingCategoryId(
        type,
        parameters,
        categoryId,
      );
  ...
  async validate(input: CategoryInput, categoryId?: number): Promise<void> {
    ...
    await this.validateDuplicateNames(
      input.type,
      input.translations,
      categoryId,
    );
