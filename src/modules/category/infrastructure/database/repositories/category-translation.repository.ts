...
export class CategoryTranslationRepository implements ICategoryTranslationRepository {
  ...
  async getByNameAndLanguageAndTypeExcludingCategoryId(
    type: CategoryTypeEnum,
    translations: { language: LanguageEnum; name: string }[],
    categoryId?: number,
  ): Promise<GetCategoryTranslationByNameViewModel[]> {
    const qb =
      this.categoryTranslationRepository.createQueryBuilder('translations');
    ...
    const categoryTranslationEntities = await qb
      .select(['translations.language', 'translations.name'])
      .getMany();

    return categoryTranslationEntities.map(({ language, name }) => ({
      language,
      name,
    }));
  }
  ...
  async saveMany(
    ...
    const categoryTranslationRepository =
      manager?.getRepository(CategoryTranslationEntity) ??
