...
export class CategoryController {
  ...
  async getAllCategoriesPaginatedWithCount(
    ...
    const { models: data, count } =
      await this.getAllCategoriesPaginatedWithCountUseCase.execute(
        { ...query, language },
        order,
        pagination,
      );
  ...
  async getByLanguage(
    ...
    const categoryTranslationModels =
      await this.getCategoryTranslationsByLanguageUseCase.execute(language);
    return categoryTranslationModels.map(
      (categoryTranslationModel) =>
        new CategoryListResponseDto(categoryTranslationModel),
    );
  }
