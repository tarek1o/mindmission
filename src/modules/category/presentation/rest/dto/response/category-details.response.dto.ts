...
export class CategoryDetailsResponseDto {
  ...
  constructor(categoryWithTranslationsViewModel: CategoryWithTranslationsViewModel) {
    const { category, translations } = categoryWithTranslationsViewModel;
    this.id = category.id;
    this.translations = translations.map(translation => new CategoryTranslationResponseDto(translation));
