...
import {
  IActionTokenConfiguration,
  TokenDurationLifeTime,
} from 'src/infrastructure/configuration/interfaces/sub-interfaces/action-token-config.interface';
...
export class StatefulTokenStrategy implements ITokenStrategy {
  constructor(
    ...
    private readonly configService: ConfigService<
      ...
      true
    >,
    @Inject(ACTION_TOKEN_REPOSITORY)
    private actionTokenRepository: IActionTokenRepository,
  ) {}

  private getExpirationDate(type: ActionTokenTypeEnum): Date | null {
    const durationConfig: TokenDurationLifeTime =
      this.configService.get<IActionTokenConfiguration>('actionTokens')
        .stateful[type];
    return durationConfig
      ? moment().add(durationConfig.value, durationConfig.unit).toDate()
      : null;
  }
  ...
  async generate<T>(
    options: { type: ActionTokenTypeEnum; payload: T; userId?: number },
    manager?: unknown,
  ...
  async verify(
    ...
    const actionToken = uuid
      ? await this.actionTokenRepository.getByTokenAndUUID(type, token, uuid)
      : await this.actionTokenRepository.getByToken(type, token);
  ...
  getActiveTokensByUserIdAndType(
    ...
    return this.actionTokenRepository.getActiveTokensByUserIdAndTokenType(
      userId,
      type,
    );
  }
  ...
  async revokeMany(
    ...
    validActionTokens.length &&
      (await this.actionTokenRepository.saveMany(validActionTokens, manager));
  }
}
