...
export class StatelessTokenStrategy implements ITokenStrategy {
  ...
  private getStatelessTokenConfig(
    type: ActionTokenTypeEnum,
  ): StatelessTokenConfig {
    const statelessTokenConfig: StatelessTokenConfig =
      this.configService.get<IActionTokenConfiguration>('actionTokens')
        .stateless[type];
    if (!statelessTokenConfig) {
      throw new Error(
        `No config has been provided for ${type} stateless token`,
      );
    }
  ...
  private getExpirationDate(type: ActionTokenTypeEnum): Date | null {
    ...
    return expiresIn
      ? moment().add(expiresIn.value, expiresIn.unit).toDate()
      : null;
  }
  ...
  async verify(
    ...
    const { iat, exp, ...payload } = await this.jwtService.verify(token, {
      secret,
    });
    return new ActionTokenModel({
      type,
