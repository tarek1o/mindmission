...
export class OAuthProviderRepository implements IOAuthProviderRepository {
  constructor(
    @InjectRepository(OAuthProviderEntity) private readonly oauthProviderRepository: Repository<OAuthProviderEntity>,
  ) {}

  async getByProviderIdAndUserId(provider: OAuthProviderEnum, userId: number): Promise<OAuthProfileModel | null> {
  ...
  private errorHandler(error: unknown) {
    const errorMappingResult: ErrorMappingResult[] = [
      {
        constraint: OAUTH_PROVIDER_INDEXES_CONSTANTS.OAUTH_PROVIDERS_PROVIDER_ID_USER_ID_PARTIAL_UNIQUE_INDEX,
