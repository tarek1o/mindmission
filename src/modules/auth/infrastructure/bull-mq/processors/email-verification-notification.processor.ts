...
export class EmailVerificationNotificationProcessor extends WorkerHost {
  ...
  private getContext(message: EmailVerificationNotificationMessage): Record<string, string> {
    const { emailVerificationURL } = this.configService.get<UILinks>('uiLinks').mainApp;
    const remainingTime = moment(message.expirationDate).diff(moment(), 'days', true)
    return {
  ...
  async process(job: Job<EmailVerificationNotificationMessage>): Promise<void> {
    const channel = await this.notificationChannelFinderService.getChannel(NotificationEventEnum.EMAIL_VERIFICATION);
    const context = this.getContext(job.data);
    await this.notificationSenderService.send({
      channel,
      subject: this.translationService.translate('info.notification.subject.email_verification'),
      to: job.data.email,
      context,
