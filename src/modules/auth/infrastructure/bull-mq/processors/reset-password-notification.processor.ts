...
export class ResetPasswordNotificationProcessor extends WorkerHost {
  ...
  private getContext(message: ResetPasswordNotificationMessage): Record<string, string> {
  ...
  async process(job: Job<ResetPasswordNotificationMessage>): Promise<void> {
    ...
    await this.notificationSenderService.send({
      channel,
      subject: this.translationService.translate('info.notification.subject.reset_password'),
      to: job.data.email,
      context,
