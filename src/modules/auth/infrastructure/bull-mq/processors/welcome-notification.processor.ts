...
export class WelcomeNotificationProcessor extends WorkerHost {
  ...
  private getContext(message: WelcomeNotificationMessage): Record<string, string> {
    return {
  ...
  async process(job: Job<WelcomeNotificationMessage>): Promise<void> {
    const channel = await this.notificationChannelFinderService.getChannel(NotificationEventEnum.WELCOME);
    const context = this.getContext(job.data);
    await this.notificationSenderService.send({
      channel,
      subject: this.translationService.translate('info.notification.subject.welcome'),
      to: job.data.email,
      context,
