...
export class ResetPasswordUseCase {
  ...
  private async getUserById(userId: number) {
    ...
    if (!user) {
      throw new BusinessRuleViolationError('auth.reset_password.user_not_found');
    }
    return user;
  }
  ...
  private async finalizePasswordReset(user: UserModel, actionToken: ActionTokenModel): Promise<void> {
  ...
  async execute(resetPasswordInput: ResetPasswordInput): Promise<void> {
    const actionToken = await this.actionTokenVerifierService.verifyResetPasswordToken(resetPasswordInput.token);
    const user = await this.getUserById(actionToken.userId);
    const hashedPassword = await PasswordHashingHelper.hash(resetPasswordInput.password);
