...
export class SignupUseCase {
  ...
  private async saveUserAndGenerateEmailVerificationToken(
    user: UserModel,
  ): Promise<{ user: UserModel; actionToken: ActionTokenModel }> {
    return this.unitOfWork.transaction(async (manager) => {
      const savedUser = await this.userRepository.save(user, manager);
      const actionToken = await this.generateEmailVerificationToken(
        savedUser.id,
        manager,
      );
      return {
  ...
  private async sendEmailVerificationNotification(
    user: UserModel,
  ...
  async execute(signUpDto: SignupInput): Promise<void> {
    await this.userValidator.checkEmailDuplicate(
      signUpDto.email,
      AppUiEnum.MAIN_APP,
    );
    const userModel = await this.createUserModel(signUpDto);
    const { user, actionToken } =
      await this.saveUserAndGenerateEmailVerificationToken(userModel);
