...
export class OAuthLoginFlowStrategy extends OAuthFlowStrategy {
  constructor(
    @Inject(OAUTH_PROVIDER_REPOSITORY) private readonly oauthProviderRepository: IOAuthProviderRepository,
  ) {
    super();
  }

  async handle(providerProfile: ProviderProfileInput, user: UserModel): Promise<UserModel> {
    const oAuthProvider = await this.oauthProviderRepository.getByProviderIdAndUserId(providerProfile.provider, user.id);
    if (!oAuthProvider) {
      const oAuthProviderModel = this.constructOAuthProviderModel(providerProfile, user.id);
