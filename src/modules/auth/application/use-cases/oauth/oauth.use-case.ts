...
export class OAuthUseCase {
  ...
  async execute(
    ...
    const oauthFlowStrategy =
      this.oauthFlowStrategyResolverService.resolve(user);
    const savedUser = await oauthFlowStrategy.handle(providerProfile, user);
    const accessTokenModel =
      await this.authTokenService.generateAccessToken(savedUser);
    const refreshTokenModel =
      await this.authTokenService.generateRefreshToken(savedUser);
    return {
