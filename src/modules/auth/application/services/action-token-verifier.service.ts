...
export class ActionTokenVerifierService {
  ...
  async verifyResetPasswordToken(token: string): Promise<ActionTokenModel> {
    try {
      return await this.actionTokenService.verify(ActionTokenTypeEnum.RESET_PASSWORD_TOKEN, token);
    } catch (error) {
  ...
  async verifyEmailVerificationToken(token: string): Promise<ActionTokenModel> {
    try {
      return await this.actionTokenService.verify(ActionTokenTypeEnum.EMAIL_VERIFICATION_TOKEN, token);
    } catch (error) {
      throw new BusinessRuleViolationError('auth.signup.invalid_or_expired_email_verification_token');
    }
  }

  async verifyRefreshToken(token: string): Promise<ActionTokenModel> {
    try {
      return await this.actionTokenService.verify(ActionTokenTypeEnum.REFRESH_TOKEN, token);
    } catch (error) {
