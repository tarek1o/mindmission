...
export class SuspendedAccountRepository implements ISuspendedAccountRepository {
  constructor(
    @InjectRepository(SuspendedAccountEntity) private readonly repository: Repository<SuspendedAccountEntity>,
    @Inject(LOGGER_SERVICE) private readonly logger: LoggerService,
  ) {}

  async getByRecipientAndChannel(recipient: string, reason: SuspendedReasonEnum): Promise<SuspendedAccountModel | null> {
    ...
    return suspendedAccount ? SuspendedAccountMapper.toModel(suspendedAccount) : null;
  }

  async save(suspendedAccount: SuspendedAccountModel, manager?: EntityManager): Promise<SuspendedAccountModel> {
    const entity = SuspendedAccountMapper.toEntity(suspendedAccount);
    const repository = manager?.getRepository(SuspendedAccountEntity) ?? this.repository;
    const savedEntity = await repository.save(entity);
    return SuspendedAccountMapper.toModel(savedEntity);
  }

  async cleanupExpiredSuspensions(): Promise<void> {
    const result = await this.repository.delete({ suspendedUntil: LessThanOrEqual(new Date()) });
    this.logger.log(`Deleted suspended accounts results: ${JSON.stringify(result)}`);
  }

  async delete(suspendedAccount: SuspendedAccountModel, manager?: EntityManager): Promise<void> {
    const repository = manager?.getRepository(SuspendedAccountEntity) ?? this.repository;
