...
export class ProfileController {
  ...
  async updateMainUserInfo(
    ...
    const userModel = await this.updateMainUserInfoUseCase.execute(
      user.id,
      updateMainUserInfo,
    );
    ...
    @UploadedFile(
      new CustomParseFilePipe({
        allowedTypes: Object.values(
          ImageFileEnum,
        ) as NotEmptyArray<ImageFileEnum>,
        maxSizeInMB: configService.getNumber(
          'STORAGE_IMAGE_PROFILE_IMAGES_MAX_SIZE',
        ),
        isRequired: true,
      }),
    )
