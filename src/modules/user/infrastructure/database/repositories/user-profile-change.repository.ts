...
export class UserProfileChangeRepository implements IUserProfileChangeRepository {
  constructor(
    @InjectRepository(UserProfileChangeEntity) private readonly userProfileChangeRepository: Repository<UserProfileChangeEntity>,
  ...
  async getByUserIdAndField(userId: number, field: UserProfileFieldEnum): Promise<UserProfileChangeModel | null> {
  ...
  async save(userProfileChangeModel: UserProfileChangeModel, manager?: EntityManager): Promise<UserProfileChangeModel> {
    try {
      const entity = UserProfileChangeMapper.toEntity(userProfileChangeModel);
      const userProfileChangeRepository = manager?.getRepository(UserProfileChangeEntity) ?? this.userProfileChangeRepository;
