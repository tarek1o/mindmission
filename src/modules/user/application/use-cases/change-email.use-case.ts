...
export class ChangeEmailUseCase {
  ...
  private async validatePassword(user: UserModel, password: string): Promise<void> {
    ...
    if (!isPasswordValid) {
      throw new BusinessRuleViolationError('user.email.change.incorrect_password');
    }
  }

  private async createEmailChangeRequest(user: UserModel, newEmail: string): Promise<UserProfileChangeModel> {
    const userProfileChange = await this.userProfileChangeRepository.getByUserIdAndField(user.id, UserProfileFieldEnum.EMAIL);
  ...
  private async generateChangeEmailToken(userId: number, manager: unknown): Promise<ActionTokenModel> {
    const payload = {
      userId,
    }
    return this.actionTokenService.generate(ActionTokenTypeEnum.CHANGE_EMAIL, payload, userId, manager);
  }

  private async saveUserProfileChangeAndGenerateChangeEmailToken(userProfileChange: UserProfileChangeModel): Promise<ActionTokenModel> {
    return this.unitOfWork.transaction(async (manager) => {
      const savedUserProfileChange = await this.userProfileChangeRepository.save(userProfileChange, manager);
      return await this.generateChangeEmailToken(savedUserProfileChange.userId, manager);
    })
  }

  private async sendChangeEmailVerification(user: UserModel, actionToken: ActionTokenModel): Promise<void> {
