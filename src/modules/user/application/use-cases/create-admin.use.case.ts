...
export class CreateAdminUseCase {
  ...
  private async saveUserAndGenerateSetPasswordToken(
    user: UserModel,
  ...
  private async sendSetFirstPasswordNotification(
    user: UserModel,
  ...
  async execute(createUserDto: CreateUserInput): Promise<UserModel> {
    ...
    const roles = await this.userValidatorService.validateRoles(
      createUserDto.rolesIds,
    ); // TODO: use role validator service in role module
    const userModel = await this.createUserModel(createUserDto, roles);
    const { user, actionToken } =
      await this.saveUserAndGenerateSetPasswordToken(userModel);
