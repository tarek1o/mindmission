...
export class CompleteChangeEmailUseCase {
  ...
  private async verifyChangeEmailToken(
    token: string,
  ...
  private async getEmailChangeRequest(
    id: number,
  ): Promise<UserProfileChangeModel> {
    const userProfileChange =
      await this.userProfileChangeRepository.getById(id);
  ...
  async execute(token: string): Promise<void> {
    ...
    const userProfileChange = await this.getEmailChangeRequest(
      actionToken.payload.userProfileChangeId,
    );
    const user = await this.userFinderService.getById(actionToken.userId);
    await this.userValidatorService.checkEmailDuplicate(
      userProfileChange.newValue,
      user.appUi,
    );
